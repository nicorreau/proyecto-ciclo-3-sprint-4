...
function mostrarHistorial(motocicletas) {
    let contenido = "";
    if (motocicletas.length >= 1) {
        $.each(motocicletas, function (index, motocicleta) {
            ...
            contenido += '<tr><th scope="row">' + motocicleta.id + '</th>' +
                    ...
                    '<td><input type="checkbox" name="novedad" id="novedad' + motocicleta.id 
                    + '" disabled ';
            if (motocicleta.novedad) {
                contenido += 'checked'
            }
            contenido += '></td><td>' + motocicleta.fechaAlquiler + '</td>' +
                    '<td><button id="devolver-btn" onclick= "devolvermotocicleta(' + motocicleta.id 
                    + ');" class="btn btn-danger">Devolver motocicleta</button></td></tr>';
...
function devolverMotocicleta(id) {

    $.ajax({
        type: "GET",
        dataType: "html",
        url: "./ServletMotocicletaDevolver",
        data: $.param({
            username: username,
            id: id,
        }),
